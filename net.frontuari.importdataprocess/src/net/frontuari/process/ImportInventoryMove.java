 ...
 *  @author		Jorge Colmenarez, jcolmenarez@frontuari.net, www.frontuari.net
...
public class ImportInventoryMove extends FTUProcess {
	...
	private boolean importMInventoryMoveLine(MMovement move, X_I_Movement imove)
	{
		...
		try
		{
			...
			//	Added By Jorge Colmenarez, 2021-11-12 10:31
			//	Support for import ASI & ASITo
			if(imove.get_ValueAsInt("M_AttributeSetInstance_ID")>=0)
				moveLine.setM_AttributeSetInstance_ID(imove.get_ValueAsInt("M_AttributeSetInstance_ID"));
			if(imove.get_ValueAsInt("M_AttributeSetInstanceTo_ID")>=0)
				moveLine.setM_AttributeSetInstanceTo_ID(imove.get_ValueAsInt("M_AttributeSetInstanceTo_ID"));
			//	Added By Jorge Colmenarez, 2021-11-12 10:51
			//	Support for OrgTarget
			moveLine.set_ValueOfColumn("AD_OrgTarget_ID", imove.getAD_OrgTrx_ID());
			//	Support for Cumulated MovementQty
			moveLine.setMovementQty(moveLine.getMovementQty().add(imove.getMovementQty()));
	...
	/**
	 * get FTUMovementLine unique instance based on  X_I_M_Movement data
	 * @param move MMovement
	 * @param imove X_I_M_Movement
	 * @return  unique instance of FTUMovementLine
	 */
	private MMovementLine getMInventoryMoveLine(MMovement move, X_I_Movement imove)
	{
		...
		{			
			...
			//	Added By Jorge Colmenarez, 2021-11-12 10:27
			//	Add to filter unique ASI & ASITo
			|| "M_AttributeSetInstance_ID".equals(col.getColumnName())
			|| "M_AttributeSetInstanceTo_ID".equals(col.getColumnName()))
			{
				if("M_AttributeSetInstanceTo_ID".equals(col.getColumnName()) 
						&& imove.get_Value(col.getColumnName()) == null)
					whereClause.append(col.getColumnName()).append(" IS NULL");
				else
				{
					whereClause.append(col.getColumnName()).append("=?");
					parameters.add(imove.get_Value(col.getColumnName()));
				}
				//	Change count limit from 3 to 5
				if(count < 5)
				//	End Jorge Colmenarez
				{
	...
	private void fillIDValues()
	{
		...
		for(X_I_Movement imove : getRecords(false, false))
		{
			...
			//	Added by Jorge Colmenarez, 2021-11-12 10:19
			//	Support for getASI by Lot or SerNo
			int ASI = 0;
			int ASITo = 0;
			if(imove.get_ValueAsInt("M_AttributeSetInstance_ID") == 0 && imove.get_ValueAsString("Lot") != null)
			{
				ASI = getID(MAttributeSetInstance.Table_Name,"Lot = ?",new Object[]{imove.get_ValueAsString("Lot")});
				if(ASI >= 0)
					imove.set_ValueOfColumn("M_AttributeSetInstance_ID", ASI);
			}
			if(imove.get_ValueAsInt("M_AttributeSetInstance_ID") == 0 && imove.get_ValueAsString("SerNo") != null)
			{
				ASI = getID(MAttributeSetInstance.Table_Name,"SerNo = ?",new Object[]{imove.get_ValueAsString("SerNo")});
				if(ASI >= 0)
					imove.set_ValueOfColumn("M_AttributeSetInstance_ID", ASI);
			}
			if(imove.get_ValueAsInt("M_AttributeSetInstanceTo_ID") == 0 && imove.get_ValueAsString("LotTo") != null)
			{
				ASITo = getID(MAttributeSetInstance.Table_Name,"Lot = ?",new Object[]{imove.get_ValueAsString("LotTo")});
				if(ASITo >= 0)
					imove.set_ValueOfColumn("M_AttributeSetInstanceTo_ID", ASITo);
			}
			if(imove.get_ValueAsInt("M_AttributeSetInstanceTo_ID") == 0 && imove.get_ValueAsString("SerNoTo") != null)
			{
				ASI = getID(MAttributeSetInstance.Table_Name,"SerNo = ?",new Object[]{imove.get_ValueAsString("SerNoTo")});
				if(ASITo >= 0)
					imove.set_ValueOfColumn("M_AttributeSetInstanceTo_ID", ASITo);
			}
			if(ASI<0)
				imove.set_ValueOfColumn("M_AttributeSetInstance_ID", 0);
			if(ASITo<0)
				imove.set_ValueOfColumn("M_AttributeSetInstanceTo_ID", null);
			//	End Jorge Colmenarez
