...
public class ImportPriceList extends SvrProcess
{
	...
	/** Added By Jorge Colmenarez 2020-08-10 11:00
	 * Allocate LPC to Vendor 			*/
	private boolean			p_SetBPPriceList = false;
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			//	Added by Jorge Colmenarez 2020-08-10 11:01
			else if (name.equals("SetBPPriceList"))
				p_SetBPPriceList = "Y".equals(para[i].getParameter());
			else
	...
	protected String doIt() throws Exception
	{
		...
		*/
		...
		//	Added by Jorge Colmenarez 2020-08-10 11:06, Valide set BreakValue
		if(!p_SetBPPriceList)
		{
			sql = new StringBuilder ("UPDATE I_PriceList ")
		...
		try
		{
			...
			while (rs.next())
			{
				...
				//	Modified by Jorge Colmenarez 2020-08-10 11:07, set when not BPPriceList
				if (imp.getC_BPartner_ID() > 0 && !p_SetBPPriceList) {
				...
				//	Added by Jorge Colmenarez 2020-08-10 11:08, set BP PriceList 
				//	Update PriceList for BPartner
				if(p_SetBPPriceList && imp.getC_BPartner_ID()>0)
				{
					MBPartner bp = new MBPartner(imp.getCtx(),imp.getC_BPartner_ID(),get_TrxName());
					if(imp.isSOPriceList() && bp.getM_PriceList_ID() != imp.getM_PriceList_ID())
					{
						bp.setM_PriceList_ID(imp.getM_PriceList_ID());
						bp.saveEx(get_TrxName());
					}
					else if(!imp.isSOPriceList() && bp.getPO_PriceList_ID() != imp.getM_PriceList_ID())
					{
						bp.setPO_PriceList_ID(imp.getM_PriceList_ID());
						bp.saveEx(get_TrxName());
					}
				}
