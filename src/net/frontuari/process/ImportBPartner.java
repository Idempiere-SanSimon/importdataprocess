...
//		Existing Location ? Exact Match
		...
		sql = new StringBuilder ("UPDATE I_BPartner i ")
				...
				.append(" WHERE i.TaxPayerTypeName=tpt.Name AND tpt.AD_Client_ID IN (0, i.AD_Client_ID)) ")
				.append("WHERE LCO_TaxPayerType_ID IS NULL AND TaxPayerTypeName IS NOT NULL")
				.append(" AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdateEx(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.FINE)) log.fine("Set Tax Payer Type=" + no);
		//
		sql = new StringBuilder ("UPDATE I_BPartner i ")
				.append("SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'ERR=Invalid Tax Payer Type, ' ")
				.append("WHERE LCO_TaxPayerType_ID IS NULL AND TaxPayerTypeName IS NOT NULL")
				.append(" AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdateEx(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.CONFIG)) log.config("Invalid Tax Payer Type=" + no);

		//	Set ISIC/CIIU
		sql = new StringBuilder ("UPDATE I_BPartner i ")
				.append("SET LCO_ISIC_ID=(SELECT LCO_ISIC_ID FROM LCO_ISIC isic")
				.append(" WHERE i.ISICValue=isic.Value AND isic.AD_Client_ID IN (0, i.AD_Client_ID)) ")
				.append("WHERE LCO_ISIC_ID IS NULL AND ISICValue IS NOT NULL")
				.append(" AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdateEx(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.FINE)) log.fine("Set ISIC/CIIU=" + no);
		//
		sql = new StringBuilder ("UPDATE I_BPartner i ")
				.append("SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'ERR=Invalid ISIC/CIIU, ' ")
				.append("WHERE LCO_ISIC_ID IS NULL AND ISICValue IS NOT NULL")
				.append(" AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdateEx(sql.toString(), get_TrxName());
	...
	private void setTypeOfBPartner(X_I_BPartner impBP, MBPartner bp){
		...
		//	Set IsShareHolder 
		if(impBP.get_ValueAsBoolean("IsShareHolder"))
		{
